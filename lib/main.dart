...
//import 'package:wristband/screens/sensor_data_screen.dart';
import 'package:wristband/sensor_data_screen.dart';
...
Future<Map<String, dynamic>> retrieveData() async {
  DatabaseReference ref = FirebaseDatabase.instance.ref("patientData"); // Reference to Firebase data
  try {
                ...
                children: [
                  Container(
                    ...
                    child: Column(
                      children: [
                        Text(
                          heartRate > 0 ? "routes: {'/sensorData': (context) => SensorDataScreen(), }, ❤️" : "No Heart Rate Data",  // Add SensorDataScreen route
                          style: TextStyle(
                            color: Colors.red,
                            fontSize: 50,
...
class _PatientMonitoringScreenState extends State<PatientMonitoringScreen> {
  ...
  void _updateData() {
    DatabaseReference dbRef = FirebaseDatabase.instance.ref("health_data");

    dbRef.limitToLast(1).onValue.listen((event) {
      if (event.snapshot.value != null) {
        Map<dynamic, dynamic> data = event.snapshot.value as Map<dynamic, dynamic>;
        var latestData = data.values.last; // Get the latest entry
                    ...
                    children: [
                      ...
                      Column(
                        ...
                        children: [
                          ...
                          Text("Name: ${widget.relativeName}"),
                          Text("Name of Patient: ${widget.patientName}"),
                          Text("Contact: ${widget.relativeContact}"),
                        ],
                      ),
                    ],
                  ),
